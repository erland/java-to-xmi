...
public final class JavaExtractor {
    ...
    static void propagateJpaRelationshipAnnotationsFromGettersToFields(
            List<se.erland.javatoxmi.model.JField> fields,
            List<se.erland.javatoxmi.model.JMethod> methods
    ) {
        ...
        for (int i = 0; i < fields.size(); i++) {
            se.erland.javatoxmi.model.JField f = fields.get(i);
            if (f == null) continue;
            String n = f.name == null ? "" : f.name.trim();
            if (!n.isEmpty() && !fieldIndex.containsKey(n)) fieldIndex.put(n, i);
        }

        for (se.erland.javatoxmi.model.JMethod m : methods) {
            if (m == null) continue;
            ...
            List<se.erland.javatoxmi.model.JAnnotationUse> relAnns = m.annotations.stream()
            ...
            se.erland.javatoxmi.model.JField f = fields.get(idx);
            if (f == null) continue;

            List<se.erland.javatoxmi.model.JAnnotationUse> merged = mergeAnnotations(f.annotations, relAnns);
            if (merged == f.annotations || merged.equals(f.annotations)) continue;

            fields.set(idx, new se.erland.javatoxmi.model.JField(f.name, f.type, f.typeRef, f.visibility, f.isStatic, f.isFinal, merged));
        }
    }

    private static String getterPropertyName(se.erland.javatoxmi.model.JMethod m) {
    ...
    private static boolean isJpaRelationshipOrJoinAnnotation(se.erland.javatoxmi.model.JAnnotationUse a) {
    ...
    private static List<se.erland.javatoxmi.model.JAnnotationUse> mergeAnnotations(
            List<se.erland.javatoxmi.model.JAnnotationUse> base,
            List<se.erland.javatoxmi.model.JAnnotationUse> extra
    ) {
        if (extra == null || extra.isEmpty()) return base == null ? List.of() : base;
        List<se.erland.javatoxmi.model.JAnnotationUse> out = new ArrayList<>();
        if (base != null) out.addAll(base);

        // Track existing by qualifiedName when present, else by simpleName.
        Set<String> seen = new HashSet<>();
        for (se.erland.javatoxmi.model.JAnnotationUse a : out) {
            if (a == null) continue;
            String key = (a.qualifiedName != null && !a.qualifiedName.isBlank()) ? a.qualifiedName : a.simpleName;
            if (key != null && !key.isBlank()) seen.add(key);
        }
        for (se.erland.javatoxmi.model.JAnnotationUse a : extra) {
            if (a == null) continue;
            String key = (a.qualifiedName != null && !a.qualifiedName.isBlank()) ? a.qualifiedName : a.simpleName;
